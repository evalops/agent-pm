...
def _build_parser() -> argparse.ArgumentParser:
    ...
    reload_parser.add_argument(
        "name", nargs="?", help="Optional plugin name for targeted reload"
    )
    ...
    install_parser = sub.add_parser(
        "install", help="Install a plugin from an entry point or module reference"
    )
    install_parser.add_argument(
        "--entry-point",
        dest="entry_point",
        help="Entry point name (agent_pm.plugins group)",
    )
    install_parser.add_argument(
        "--module", help="Module reference in module:Class format"
    )
    install_parser.add_argument("--name", help="Override plugin name")
    install_parser.add_argument(
        "--enable", action="store_true", help="Enable plugin after installation"
    )
    install_parser.add_argument(
        "--set",
        metavar="KEY=VALUE",
...
def main() -> None:
    ...
    if args.command == "list":
        ...
        for item in metadata:
            ...
            if missing:
                print(
                    f"[hint] {name}: missing secrets -> {', '.join(missing)}",
                    file=sys.stderr,
                )
        ...
        for item in entry_points:
            if item.get("error"):
                print(
                    f"[warning] {item['entry_point']}: {item['error']}", file=sys.stderr
                )
        ...
        if entry_point and not module:
            catalogue = {
                item["entry_point"]: item for item in registry.discover_plugins()
            }
